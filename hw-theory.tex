...
(a) существует формула $\alpha$, что $\vdash\alpha\with\neg\alpha$;
...
следует $\vdash\varphi$. Покажите требуемое утверждение конструктивно, перестроив данные в условии доказательства в доказательство $\varphi$. 
...
А как закодировать число, если мы не знаем верхней границы $n$? Какую лучшую ассимптотику длины кодировки относительно $\log_2 k$ вы можете
предложить?
...
Естественно, символы алфавитов при кодировке меняются на их номера, и их надо будет как-то занумеровать в виде последовательностей цифр 0 и 1.
\end{enumerate}
