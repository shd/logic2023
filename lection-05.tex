...
ни одно свободное вхождение переменных в $\psi$ не станет связанным после подстановки.\end{dfn}
...
\begin{frame}{Корректность}
\begin{thm}Если $\Gamma \vdash \alpha$ и в доказательстве не используются кванторы по переменным из $FV(\Gamma)$, то $\Gamma \models \alpha$\end{thm}
